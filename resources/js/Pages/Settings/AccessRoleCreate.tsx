...
/**
 * Checkpoint 28 — name/description only. There is no slug field, no
 * "system role" toggle, no tenant picker, no permission picker on this
 * form — a new role is always created as a custom (is_system_role:
 * false), current-tenant role with zero permissions, and permissions
 * are added afterward on the detail page, one at a time.
 */
export default function SettingsAccessRoleCreate() {
    ...
    const submit: FormEventHandler = (e) => {
        ...
        api.post<{ data: Role }>('/roles', payload)
            .then((response) => {
                router.visit(`/settings/access/roles/${response.data.data.id}`);
            })
            .catch((err) => {
    ...
    return (
        <AppLayout>
            ...
            <PageHeader
                title="Create role"
                description="New roles start with no permissions — add them from the role's page after creating it."
                actions={
