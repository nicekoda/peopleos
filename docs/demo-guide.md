...
- Custom Fields (Checkpoint 48) support Recruitment Applicants only — job openings/applications' own fields, employees, leave, onboarding/offboarding, and every other entity have no custom-field support yet (documented roadmap: `job_applications` next, then `lifecycle_processes`/`leave_requests`, `employees` last). No field-level read permission — sensitivity classification only affects audit-log masking today, not who can see a value. No custom-forms builder, workflow conditions, reports, or dashboards consuming custom fields yet — this is the storage/definition foundation only.
