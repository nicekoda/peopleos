...
- **HR Documents & Letter Generation** — no demo data is pre-seeded (deliberately, same reasoning as Onboarding & Offboarding); create it live instead. As Tenant Admin or HR Manager, go to Settings → HR Document Templates, create a simple Employment Letter template using a couple of placeholders (e.g. `Dear {{employee.name}}, this confirms your role as {{employee.position}} in {{employee.department}}, effective {{employee.start_date}}.`) — this creates the template and its first published version together. Open the template again to show the Versions card (Checkpoint 36): click "Create new version", tweak the wording, save the draft, then "Publish" it — show the previous version move to "archived" (not deleted) and the new one become "(current)". Go to `/hr-documents/create`, pick an employee and the template, and generate — show the rendered letter uses the *currently published* version's wording. Click "Download PDF" on the generated document's detail page (Checkpoint 35) to show the same content as a real downloadable PDF file, generated on the spot — nothing is stored on disk. Switch to Employee or Line Manager and show `/hr-documents` and `/settings/hr-document-templates` are both inaccessible (403) — HR letters are an HR-administrative function, not self-service, by deliberate design this checkpoint.
...
- HR Documents & Letter Generation (Checkpoint 34) has no DOCX file, e-signature, approval workflow before a letter is final, automated sending, bulk generation, or employee self-service download. PDF download was added in Checkpoint 35 — generated on demand, never stored on disk. Template version history was added in Checkpoint 36 — no diff/compare view between versions, and no approval step before publishing.
