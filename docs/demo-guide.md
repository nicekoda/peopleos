...
- Custom Fields support Recruitment Applicants (Checkpoint 48) and Job Applications (Checkpoint 49) only — job openings, employees, leave, onboarding/offboarding, and every other entity have no custom-field support yet (documented roadmap: `lifecycle_processes`/`leave_requests` next, `employees` last). Adding Job Applications as the second entity required no schema or core-service change — proof the engine generalizes. No field-level read permission — sensitivity classification only affects audit-log masking today, not who can see a value. No custom-forms builder, workflow conditions, reports, or dashboards consuming custom fields yet — this is the storage/definition foundation only. Converting an application to an employee never copies its custom field values — Employee custom fields don't exist yet.
