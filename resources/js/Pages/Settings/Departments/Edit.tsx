 ...
 * existing department may need to deactivate it without archiving it
 * entirely. tenant_id/created_by/updated_by/deleted_at/slug remain
 * structurally absent.
 */
export default function SettingsDepartmentEdit() {
    const { departmentId } = usePage<EditProps>().props;

    const [loadError, setLoadError] = useState<ApiError | null>(null);
    const [form, setForm] = useState<DepartmentFormPayload | null>(null);
    ...
    useEffect(() => {
        api.get<{ data: Department }>(`/departments/${departmentId}`)
            .then((response) => {
    ...
    const set = <K extends keyof DepartmentFormPayload>(key: K, value: DepartmentFormPayload[K]) => {
    ...
    const submit: FormEventHandler = (e) => {
        ...
        api.patch<{ data: Department }>(`/departments/${departmentId}`, form)
            .then(() => setSuccessMessage('Department updated.'))
            .catch((err) => {
                ...
                <Card>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <SelectField
                            ...
                            name="status"
                            value={form.status}
                            onChange={(e) => set('status', e.target.value as DepartmentFormPayload['status'])}
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        <Button type="button" variant="secondary" onClick={() => router.visit('/settings/departments')}>
                            Back to list
                        </Button>
