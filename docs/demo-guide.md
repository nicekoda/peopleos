...
- Custom Fields support Recruitment Applicants (Checkpoint 48) and Job Applications (Checkpoint 49) only — job openings, employees, leave, onboarding/offboarding, and every other entity have no custom-field support yet (documented roadmap: `lifecycle_processes`/`leave_requests` next, `employees` last). Adding Job Applications as the second entity required no schema or core-service change — proof the engine generalizes. Field-level access is now enforced (Checkpoint 50) via three fixed permissions (`custom_fields.access_sensitive`/`access_confidential`/`access_restricted`) — Tenant Admin holds all three, HR Manager holds `access_sensitive` only, HR Director holds none by default (an intentional MVP decision, not an oversight); no tenant-configurable visibility rules exist yet. No custom-forms builder, workflow conditions, reports, or dashboards consuming custom fields yet — this is the storage/definition foundation only. Converting an application to an employee never copies its custom field values — Employee custom fields don't exist yet.
