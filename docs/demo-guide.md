...
- **HR Officer** — leave (tenant-wide) and policies, Onboarding/Offboarding (create/update/assign/complete, no delete), HR document templates (view only) and generated documents (view/create/generate/update/submit, no delete, and — deliberately — no approve/reject), Settings visible (for Leave Types/Departments/Positions/Locations — view/create/update, no delete), but not Users & Access, Document Categories management, or Audit Log.
